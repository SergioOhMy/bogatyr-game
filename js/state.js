export const state = {
    ...
    battleOver: false,
    // Действие текущего хода уже запущено (до следующего startTurn). Без него
    // в паузе перед сменой хода можно было снова выбрать цель и ударить ещё раз.
    actionLocked: false,
    // Автобой: 'off' | 'normal' | 'fast'. На ходу самого игрока (не бота и не
    // помощника - у них уже есть свой автоход) заменяет ожидание клика на
    // тот же ИИ, что выбирает действия ботам (см. combat.js -> autoPlayerLogic).
    // 'fast' дополнительно сжимает паузы между действиями (см. battleDelay).
    autoBattleMode: 'off'
};
